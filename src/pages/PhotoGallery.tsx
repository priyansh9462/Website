...
const PhotoGallery = () => {
  ...
  useEffect(() => {
    // Animate gallery items when category changes
  ...
  const handleCategoryChange = (categoryId: string) => {
    ...
    imageRefs.current = []; // Reset refs for new animation
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {categories.map((category) => (
              <Button
                ...
                className="filter-button hover:scale-105 transition-all duration-300 shadow-lg"
              >
                {category.name}
              </Button>
            ))}
          ...
          <div className={`grid gap-6 ${
            ...
            {filteredImages.map((image, index) => (
              <Card 
                ...
                ref={addImageRef}
                className="group cursor-pointer hover:shadow-2xl transition-all duration-500 hover:-translate-y-3 overflow-hidden"
      ...
      {/* Lightbox Modal */}
      {selectedImage && (
        <div className="lightbox-modal fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4">
          <div className="relative max-w-6xl w-full">
            <Button
              variant="ghost"
              size="icon"
              className="absolute top-4 right-4 z-10 text-white hover:bg-white/20 hover:scale-110 transition-all"
