...
const Events = () => {
  ...
  useEffect(() => {
    ...
    // Hero section entrance with improved visibility
    ...
    // Past events header animation with enhanced effects
    gsap.fromTo(pastRef.current,
    ...
    // Past events cards animation with stagger effect
  ...
  const handleViewHighlights = (eventId: number) => {
    gsap.to(window, {
      ...
      onComplete: () => {
        ...
        // Add your highlights functionality here
      }
    });
  };
        ...
        <div className="container mx-auto px-4">
          ...
          <div ref={pastGridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {pastEvents.map((event) => (
              <Card 
                key={event.id} 
                className={`hover-scale group cursor-pointer overflow-hidden transition-all duration-300 hover:shadow-2xl ${
                  event.featured ? 'featured-event ring-2 ring-primary shadow-xl' : ''
                }`}
              >
                {/* Event Image */}
                ...
                <CardFooter className="space-y-2">
                  <Button 
                    variant="default" 
                    className="w-full event-button bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 transition-all duration-300"
