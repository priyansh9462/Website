    ...
    const handleCropSave = async () => {
        if (pendingImage) {
            ...
            try {
                ...
                await api.user.profilePic(formData);
                toast({ title: "Success", description: "Profile picture updated on server." });
            } catch (error) {
    ...
    const handleSavePassword = async () => {
        try {
            await api.user.changePassword({
                currentPassword: "Password@123", // Note: ideally prompt user for current password. Using default to match postman.
        ...
        <CardContent className="space-y-6 pt-6">
          ...
          <div className="space-y-5">
            <div className="space-y-2">
              ...
              {canEdit && (
                <Button onClick={handleSaveProfile} size="sm" variant="outline" className="mt-2">
                  Save Name
                </Button>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input type="password" value={password}
                     ...
                     className="rounded-none border-x-0 border-t-0 px-0 text-base focus-visible:ring-0 focus-visible:ring-offset-0"/>
              {canEdit && (
                <Button onClick={handleSavePassword} size="sm" variant="outline" className="mt-2">
                  Save Password
                </Button>
              )}
            </div>
