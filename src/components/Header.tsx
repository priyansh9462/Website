...
const Header = () => {
  ...
  useEffect(() => {
    ...
    // Animate header elements with premium timing
      ...
      <div ref={logoRef} className="flex items-center">
        <div className="flex items-center justify-center p-5 z-[1] w-full md:w-full">
          <a href="/" className="block w-full">
            <img 
              src="ENGNEERING COLLEGE BARAN.png" 
              alt="University Logo" 
              className="h-full md:w-32 w-full min-w-[60px] object-contain translate-x-4"
            />
          </a>
        </div>
      </div>
            ...
            {navItems.map((item, index) => (
              <div key={index} className="relative group">
                {item.submenu ? (
                  <>
                    <button 
                      className="flex items-center space-x-1 text-foreground hover:text-primary transition-all duration-300 font-medium uppercase"
                    ...
                    {submenus[item.name] && (
                      <div className="absolute top-full min-w-[calc(100%+112px)] opacity-100 pb-10 transition-opacity pointer-events-auto font-normal -translate-x-10 pt-8">
                        <ul className="bg-white shadow-lg">
                          {item.submenu.map((subItem, subIndex) => (
                            <li key={subIndex} className="group/child relative whitespace-nowrap bg-white transition-colors cursor-pointer">
                              <a 
                                href={subItem.href}
                                className="flex gap-2 items-center group-hover/child:text-primary group-hover/child:font-bold transition-colors px-10 py-7"
                              >
                                {subItem.name}
                              </a>
                            </li>
                  ...
                  <a 
                    href={item.href} 
                    className="relative text-foreground hover:text-primary transition-all duration-300 font-medium uppercase"
                  >
                    {item.name}
                  </a>
                )}
         ...
         {/* Premium CTA Buttons */}
          <div ref={ctaRef} className="hidden lg:flex items-center space-x-4">
            {/* YAHAN CHANGE KIYA HAI - Student Portal se Login aur URL localhost:3000/login */}
            <Button 
              variant="outline" 
            ...
            <Button 
              ...
              className="bg-gradient-to-r from-primary to-primary/80 hover:scale-105 transition-all duration-300 shadow-lg"
            >
              Enquiry Now
            </Button>
          </div>
          ...
          <div className="lg:hidden fixed inset-0 z-40">
            ...
            <div className="absolute right-0 bg-black top-0 min-h-screen text-white w-full lg:w-1/2 md:min-w-[500px] pt-32 pb-32 px-10 xl:px-20 overflow-y-auto">
              <ul className="flex flex-col gap-4 w-full">
                {navItems.map((item, index) => (
                  <li key={index} className="max-w-96 w-full">
                    {item.submenu ? (
                      <>
                        <div 
                          className="flex justify-between items-center gap-5 text-3xl font-bold cursor-pointer transition-opacity hover:opacity-80 w-full"
                          onClick={() => toggleSubmenu(item.name)}
                        >
                          <span>{item.name}</span>
                        ...
                        {submenus[item.name] && (
                          <ul className="pl-5">
                            {item.submenu.map((subItem, subIndex) => (
                              <li key={subIndex} className="relative cursor-pointer font-bold transition-opacity hover:opacity-80 py-2">
                                <a 
                                  href={subItem.href} 
                                  className="flex items-center px-5 py-2 text-xl"
                                >
                                  {subItem.name}
                                </a>
                              </li>
              ...
              <div className="mt-10 flex flex-col gap-5">
                ...
                {/* YAHAN BHI CHANGE KIYA HAI (Mobile Menu ke liye) */}
                <Button 
                  variant="outline" 
              ...
              <div className="mt-10 flex gap-5 items-center justify-center">
                ...
                <button className="p-2 hover:scale-110 transition-transform">
                  {/* Accessibility icon would go here */}
                  <svg className="w-6 h-6" viewBox="0 0 22 22" fill="currentColor">
                    {/* SVG path for accessibility icon */}
                  </svg>
                </button>
              </div>
            </div>
