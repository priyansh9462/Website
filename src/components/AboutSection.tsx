...
const AboutSection = () => {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(imageRef.current,
      { opacity: 0, x: 150, rotationY: 15, scale: 0.8 },
      {
        opacity: 1,
        x: 0,
        rotationY: 0,
    ...
    gsap.fromTo(featuresRef.current?.children,
      { opacity: 0, y: 80, scale: 0.6, rotationX: -20 },
      {
        opacity: 1,
        y: 0,
        scale: 1,
  ...
  return (
    <section ref={sectionRef} id="about" className="py-32 bg-gradient-to-br from-background via-background/95 to-accent/10 relative overflow-hidden">
      {/* Background pattern */}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-24 items-center">
          ...
          <div ref={contentRef}>
            ...
            <div ref={featuresRef} className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {features.map((feature, index) => (
                <div key={index} className="group p-6 rounded-2xl bg-gradient-to-br from-background to-accent/5 border border-border/50 hover:border-primary/30 transition-all duration-500 hover:shadow-xl hover:scale-105">
          ...
          <div ref={imageRef} className="relative">
            <div className="relative rounded-3xl overflow-hidden shadow-2xl group">
              <img 
                src="IMG_0189 - Edited.jpg"
                alt="University Excellence"
                className="w-full h-[700px] object-cover group-hover:scale-110 transition-all duration-700"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-primary/40 via-transparent to-transparent"></div>
            </div>

            {/* Floating Stat: Years of Excellence */}
