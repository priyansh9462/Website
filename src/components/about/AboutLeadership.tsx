...
const AboutLeadership = () => {
  ...
  const renderLeaderCard = (leader: any) => (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-20 items-center p-8 rounded-3xl bg-background shadow-xl">
  ...
  return (
    <section
      ref={leadershipRef}
      className="py-32 bg-gradient-to-br from-accent/5 to-background relative overflow-hidden"
    >
      <div
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div ref={contentRef} className="space-y-28 max-w-7xl mx-auto">
          {/* Render Principal */}
          {leadershipData.filter(l => l.id !== 'vice-principal').map((leader) => (
            <div key={leader.name}>{renderLeaderCard(leader)}</div>
          ))}

          {/* Render Vice Principal Carousel with auto-slide */}
          <Carousel className="w-full">
            <CarouselContent>
              {carouselItems.map((leader, index) => (
                <CarouselItem
                  key={leader.name}
            ...
            <CarouselPrevious
              ...
              className="left-4 w-12 h-12 bg-primary/90 hover:bg-primary text-white border-0 hover:scale-110 transition-all duration-300"
            />
            <CarouselNext
              onClick={() =>
                setCurrentIndex((prev) => (prev + 1) % carouselItems.length)
              }
              className="right-4 w-12 h-12 bg-primary/90 hover:bg-primary text-white border-0 hover:scale-110 transition-all duration-300"
            />
          </Carousel>
        </div>
      </div>
