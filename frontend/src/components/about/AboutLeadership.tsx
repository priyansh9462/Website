...
const AboutLeadership = () => {
    ...
    const carouselItems = leadershipData.filter((leader) => leader.id === "vice-principal");
    useEffect(() => {
    ...
    const renderLeaderCard = (leader: any) => (<div className="grid grid-cols-1 xl:grid-cols-2 gap-20 items-center p-8 rounded-3xl bg-background shadow-xl">
      ...
      <div className="space-y-8">
        ...
        <h3 className="text-4xl md:text-5xl font-bold text-foreground">{leader.name}</h3>
        ...
        <div className="mt-12">
          <div className="p-8 rounded-3xl bg-gradient-to-br from-primary/5 to-accent/5 border border-primary/20 relative">
            ...
            <p className="text-lg font-medium text-foreground leading-relaxed italic">{leader.quote}</p>
    ...
    return (<section ref={leadershipRef} className="py-32 bg-gradient-to-br from-accent/5 to-background relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div ref={contentRef} className="space-y-28 max-w-7xl mx-auto">
          
          {leadershipData.filter(l => l.id !== "vice-principal").map((leader) => (<div key={leader.name}>{renderLeaderCard(leader)}</div>))}

          
          <Carousel className="w-full">
            <CarouselContent>
              {carouselItems.map((leader, index) => (<CarouselItem key={leader.name} className={index === currentIndex ? "block" : "hidden"}>
                  {renderLeaderCard(leader)}
                </CarouselItem>))}
            </CarouselContent>
            <CarouselPrevious onClick={() => setCurrentIndex((prev) => (prev === 0 ? carouselItems.length - 1 : prev - 1))} className="left-4 w-12 h-12 bg-primary/90 hover:bg-primary text-white border-0 hover:scale-110 transition-all duration-300"/>
            <CarouselNext onClick={() => setCurrentIndex((prev) => (prev + 1) % carouselItems.length)} className="right-4 w-12 h-12 bg-primary/90 hover:bg-primary text-white border-0 hover:scale-110 transition-all duration-300"/>
          </Carousel>
        </div>
      </div>
    </section>);
